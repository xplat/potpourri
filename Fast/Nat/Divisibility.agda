...
open import Data.Nat.Divisibility public using (_∣_; divides; quotient)
...
-- _∣_, divides, quotient from Data.Nat.Divisibility
...
poset = record
  ...
  reflexive : _≡_ ⇒ _∣_
  reflexive {n} refl = divides 1 (hsym $ proj₁ CS.*-identity n)
  ...
  trans (divides q₁ refl) (divides q₂ refl) =
    divides (q₂ * q₁) (hsym (CS.*-assoc q₂ q₁ _))
...
1∣_ : ∀ n → 1 ∣ n
1∣ n = divides n (hsym $ proj₂ CS.*-identity n)
...
∣-+ (divides {m = i} q refl) (divides q' refl) =
  divides (q + q') (hsym $ proj₂ CS.distrib i q q')
...
∣-∸ (divides {m = i} q' eq) (divides q refl) =
  divides (q' ∸ q)
          (hsym $ NatProp.im≡jm+n⇒[i∸j]m≡n q' q i _ $ hsym eq)
...
nonZeroDivisor-lemma m zero r r≢zero (divides zero eq) = r≢zero $ begin
  Fin.toℕ r
    ≡⟨ hsym $ proj₁ CS.*-identity (Fin.toℕ r) ⟩
  1 * Fin.toℕ r
    ≡⟨ eq ⟩
  0
    ∎
...
suc m ∣? .(q * suc m)         | result q (fin zero    slack fits) =
  yes $ divides q refl
suc m ∣? .(1 + r + q * suc m) | result q (fin (suc r) slack fits) =
  no $ nonZeroDivisor-lemma m q (fin (suc r) slack fits) (λ())
